...
| Path | Contents |
|------|----------|
| `packages/blocks-ui-core/` | Domain types (TrustLevel, trustLevelFromScore, CommitmentState, CommitmentRecord, commitmentStateCategory, isTerminalCommitmentState, toCommitmentRecord, toCommitmentMap), CommitmentStatePill + stateCategoryStyles (commitment-state-pill web component + category styling — promoted from commitment-viz), StatusBadge + status registry (generic status pill for all domains — lookupStatus/registerStatus with 15 built-in domains including execution/agent/pattern/conversation/epistemic, toDecoration for graph overlays), orchestration types (ExecutionState, ExecutionResult, AgentRef, AgentResult, PatternType, ExecutionModel, FailurePolicy, OrchestrationAuditEvent, ExecutionSnapshot), conversation protocol types (EpistemicStatus, ConvergenceState, ConvergenceSignal, CommonGroundState, GroundedFact, ConversationPoint, ConversationEntry, SubTaskFinding, FlagEntry, RoundMemo, ObligationChain, ConversationState), TransitionRecord (promoted from commitment-viz). Pages primitives (DataSourceMixin, emitPagesEvent, fetchSource, etc.) are imported directly from their canonical pages packages — not re-exported through blocks-ui-core. Rendering utilities (formatTimestamp, statusBorderColour, shared CSS: entryCardStyles, badgeStyles, roundDividerStyles, selectedHighlightStyles). |
| `packages/blocks-ui-schema/` | Auto-generated Zod schemas for blocks-ui component properties. BlocksComponentRegistry maps element tag names to Props interfaces. ts-morph generator reads source via tsconfig paths, produces per-component schemas + blocksComponentSchemaMap. Staleness test ensures schemas stay in sync with interfaces. Registry completeness test catches new components missing entries. |
| `components/work-item-inbox/` | Work item inbox — uses pages-table for rendering, queue pill bar, scope context bar, filter bar with counts, summary bar, three-tab perspective (My Work / Claimable / All), queue scope integration, EventStream push lifecycle |
...
| `components/list-pane/` | Generic list wrapping pages-table — DataSourceMixin data fetching, single-selection, paginated mode, client-sort/filter, selection-topic events, refresh event, optional push via EventStreamController (pushUrl + pushTopics) |
...
| `components/notification-inbox/` | Notification inbox — bell with unread badge (EventStreamController push), inbox with tabs/filters/EventStream push, subscription list CRUD, subscription editor (schema-driven form with dynamic event-type field rebuild), channel preferences (per-channel delivery mode/digest/groupBy/quiet hours), mute list (table + inline add form with scope-conditional fields), snooze control (toggle with date-time picker), notification preferences container (composes channel/mute/snooze) |
...
| `components/kpi-metric-row/` | KPI metric cards — responsive grid with sparklines, trends, status colours, density property (comfortable/compact/dense), reactive endpoint, optional push via EventStreamController (replaces polling when pushUrl set) |
...
| `components/worker-task-pane/` | Worker task pane — generic specialist task queue with context tabs (TabDefinition[]), workspace element registry (WorkspaceDefinition[]), response/claim/decline form. Extends LiveRegionMixin. Direct fetch (no DataSourceMixin) like work-item-inbox. Optional push via eventStreamEndpoint + EventStreamController. |
...
| `components/session-detail/` | Session detail — tabbed detail pane: Terminal, Git, Health, Events tabs. Tab lifecycle manages timers and EventStream push connections. |
| `components/execution-monitor/` | Execution monitor — EventStreamController-driven live execution state for orchestration framework. State badge, pattern badge, execution model summary, agent roster. Dual data mode. Staleness detection. |
...
| `packages/graph-stencil-org/` | Eidos org domain adapter — toOrgGraph (YAML→GraphModel with compound nodes), 2 stencils (org-unit container, org-agent), 6 edge types (per RelationshipKind), CST-preserving YAML editor, edit policy. Layout via LayoutEngine from pages/graph-renderer: org-specific sizing classifier, classification rules (delegates to generic defaultClassifiers), layout rules parameterized with org node types |
...
| `components/org-diagram/` | Eidos org structure diagram — editor component for organizational YAML. Extends DiagramBaseMixin. LayoutEngine-driven layout (10 strategies via ELK), org-specific toolbar with layout dropdown and archetype badge, stencil palette, property panel. Chip bar toggles (ESC/SUP/ATT/LGD) for escalation chain, supervision hierarchy, attestation grants, and legend panels. Edge hover tooltips. |
